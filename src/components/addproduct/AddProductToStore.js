...
const AddProductToStore = ({ refetchProducts }) => {
  ...
  const [purchaseSum, setPurchaseSum] = useState(true); // true = uzs, false = usd
  const [sellSum, setSellSum] = useState(true); // true = uzs, false = usd
      ...
      >
        <Form layout="vertical" form={form} onFinish={handleFinish}>
          ...
          <Row gutter={16} style={{ marginBottom: "12px" }}>
            ...
            <Col style={{ display: "flex", gap: "6px" }} span={12}>
              <p>USD</p>
              <Switch checked={sellSum} onChange={() => setSellSum(!sellSum)} />
              <p>UZS</p>
            </Col>
          </Row>
          <Row gutter={16}>
